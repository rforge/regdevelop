...
## ==================================================================
lasso.formula <-
  function(formula, data, subset, weights, na.action, model='gaussian',
...
## ==================================================================
lasso.lassogrp <- function(object, lambda=NULL, lstep=21, cv=NULL, 
                  adaptcoef = NULL, adaptlambda = NULL, ...)
{ # adaptive lasso
  if (length(object$x)==0|length(object$y)==0) {
    m <- match.call(expand.dots = FALSE)
    m$lambda <- m$coef.init <- m$penscale <- m$model <-
      m$standardize <- m$contrasts <- m$control <- m$... <- NULL
    lds <- create.design(m, formula=m$formula, env=parent.frame())
    lx <- cbind(lds$x)
    ly <- lds$y
    lindex <- lds$index
  } else {
...
## coefficients to adapt to
  ...
  if (length(lcf)==0) {
    ...
    if (length(adaptlambda)==0) {
      lcv <- if (is.null(cv)) cv.lasso(object, se=FALSE) else cv
      adaptlambda <- object$lambda[which.min(lcv$cv)]
    }
    else if (adaptlambda<0) {
      adaptlambda <- sort(object$lambda)[-adaptlambda]
    }
    lil <- which(abs(adaptlambda-object$lambda)<=0.00001*object$lambda)
    if (length(lil)==0) 
  ...
  if (length(names(lcf))==0) 
    if (length(lcf)>1)
      stop('coefficients must have names') else {
        lcf <- object$coefficients[,lcf]
        names(lcf) <- dimnames(object$coefficients)[[1]]
      }
  ## drop elements with coef==0
  lv <- is.na(lindex) | lcf!=0
  lindex <- lindex[lv]
  if (sum(!is.na(lindex))==0)
...
## --- change scales
  ...
  rslt <- lassogrp.default(lx,ly, index=lindex, model=object$model,
                           standardize=FALSE, ...)
## adjust scales
...
lassogrp.default <-
  function(x, y, index, subset, weights = rep(1, length(y)), model='gaussian',
           ...
           center=NULL, standardize = TRUE, grpnames = NULL,
...
##-   if (any(duplicated(lambdanm))) lambdanm <- as.character(lambda)
  ...
  if (length(grpnames)<max(dict.pen)) {
    grpnames <- attr(index, 'grpnames')
    if (is.null(grpnames)) grpnames <- paste('G',1:max(dict.pen),sep='.')
  }
  dimnames(norms.pen.m)[[1]] <- grpnames[dict.pen]
...
## ==================================================================
print.lassogrp <- function(x, digits = 4, coefficients=TRUE, ...)
{
  ...
  if (length(lambda)>5)
    cat(length(x$lambda),' values between ',
        format(min(x$lambda), digits = digits), ' and ',
        format(max(x$lambda), digits = digits), '\n')  else
    print(x$lambda)
  if (!is.null(x$adaptcoef)) {
    cat("* Adaptation coefficients: \n")
#    cat('  ',format(x$adaptcoef, digits = digits), '\n')
    print(x$adaptcoef)
  }
  cat("* Predictor groups      :",
      length(unique(na.omit(x$index))), "\n")
  lnm <- attr(x$index,'grpnames')
  lnmp <- dimnames(x$norms.pen)[[1]]
  cat("* Penalized predictors  :", lnmp, "\n")
  cat("  not penalized: (Intercept) ", setdiff(lnm, lnmp), "\n")
  cat("* Number of observations:", length(x$y), "\n\n")
  if (coefficients) {
    lcf <- cbind(x$coefficients)
    lsel <- ncol(lcf)>5
    ljlam <- if (lsel) round(seq(1,ncol(lcf),length=5)) else 1:ncol(lcf)
    cat("* Coefficients", if(lsel) "for selected lambdas",
        "(*: p = penalized) :\n")
    lind <- x$index
    lind[is.na(lind)] <- 0
    lord <- order(lind)
...
##-       paste(c('not pen',lnm)[lind+1], dimnames(lcf)[[1]], sep=": ")
    ...
    print(rbind('  * lambda'=x$lambda[ljlam], lcf), digits = digits)
  }
  invisible(x)
}

## ==================================================================
predict.lassogrp <- function(object, newdata, 
...
#    axis(4, mgp=c(3,2,0), at = yy[, ncol(yy)], labels = rownames(yy))
    if (legend) legendr(legend=rownames(yy),col=col,lty=lty)
## ----------------------------    
    ...
    l1coef <- apply(x$norms.pen,2,sum)
    ...
    if (!is.null(cv)) {
      ...
      if (!is.list(cv)) {
        ...
        yy <- cbind(yy, cv$cv)
        if (se) yy <- cbind(yy, cv$cv+outer(cv$cv.error,c(-1,1)))
      }
    }
    ...
    axis(3, at=sqrt(lambda), labels=rep('',length(lambda)))
    axis(2)
    par(usr=c(xlim, 0,max(l1coef)))
    lines(lambda, l1coef, col=col[4], lty=lty[4])
    axis(4, col=col[4])
    mtext('penalty',4,3)
  }
}
  ...
  function(object, i=NULL, lambda=NULL, fitfun='lm', ...)
{
  ...
  lcall <- object$call
  if (is.null(lform)) lform <- lcall$formula
  if (is.null(lform))
  ...
  if (is.null(i)) {
    ...
    i <- unique(apply( lamlam>0.99&lamlam<1.01, 1, which))
    if (is.null(i))
      stop('!extract.lassogrp!  lambda  not equal to any lambda in lassogrp object')
  }
  ni <- length(i)
  result <- NULL
  lmod <- lcall$model
  ...
  for (li in seq_along(i)) {
    ...
    lcl <- lreg$call
    lcl[1] <- 'regr.lassogrp'
    attr(lcl,'comment') <- 'call not available'
    lr$call <- lcl
    lrss <- sum(lrsd^2, na.rm=TRUE)
    lr$sigma <- sqrt(lrss/lreg$df.residual)
...
print.lassofit <-
  function(x, dummycoef = NULL, digits = max(3, options("digits")[[1]] - 3),
    doc = options("doc")[[1]], residuals=FALSE, ...)
{
  ## Purpose:
  ## ----------------------------------------------------------------------
  ## Arguments:
...
## ===================================================================
cv.lasso <- 
  function (object, K = 10, # lambda,
            #fraction = seq(from = 0, to = 1, length = 100),
            trace=FALSE, control=lassoControl(trace=0),
  ...
  ## modified cv.lars, WSt
{
  ...
  # cv.folds(length(y), K)
  residmat <- matrix(0, length(object$lambda), K)
  for (i in seq(K)) {
    omit <- all.folds[[i]]
    fit <- lassogrp.default(x, y, object$index, subset=-omit,
                            weights=object$weights, model=object$model,
                            offset=object$offset, lambda=object$lambda,
                            penscale=object$penscale,
                            center=object$innercall$center,
           ...
           boot=FALSE, seed=0, robust=TRUE)
{
  ...
  ##   boot       if FALSE, cross validation will be performed
  ##              if a number >=1,  boot  is the number of bootstrap replicates
  ##              otherwise,  100  bootstrap replicates will be performed
  ##   robust     if TRUE, means will be 20% trimmed, standard deviations
  ##              will be replaced by the robust Qn estimator
  ## Value:
  ##   coef       cv or bootstrap means and standard errors of
  ##              model coefficients
...
#  on.exit(browser())
  ...
  lblocks <- factor(if (length(blocks)==1) data[,blocks] else blocks)
  ...
  lnbl <- length(lbli)
  lboot <- (is.logical(boot)&&boot)|is.numeric(boot)
  if(lboot) {
    lnrep <- ifelse(is.numeric(boot),boot[1],100) 
    if (lnrep<=1) lnrep <- 100
  } else lnrep <- lnbl
  ## ---
  ...
  rcomp <- matrix(NA,lnrep,length(model[[comp]]),
                dimnames=list(NULL,names(model[[comp]])))
  rpress <- NULL
  rfit <- NULL
  ## -----------------------
  for (lrep in 1:lnrep) {
    if (lboot) {
      set.seed(seed+lrep)
      lbs <- sample(1:lnbl,replace=TRUE)
      li <- unlist(lbli[lbs])
    } else {
      li <- -lbli[[lrep]]
    ...
    rcomp[lrep,] <- model[[comp]]
    ## --- out of sample prediction
    if (lboot) {
      lbo <- which(!(1:lnbl)%in%lbs)
      lio <- unlist(lbli[lbo])
    } else {
      lbo <- lrep
      lio <- lbli[[lbo]]
    ...
    if (!boot) {
##-       lcfb1 <- as.character(ldt[1,blocks])
##-       rfit <- c(rfit,lfit-(lr$coef[1]+lr$allcoef[[blocks]][lcfb1])+
##-                 (model$coef[1]+model$allcoef[[blocks]][lcfb1]))
##-       lfit <- update(model, formula=...
      rfit <- c(rfit,lfit-mean(lfit)+mean(lyy[lio]))
    }
    lpres <- lyy[lio] - lfit
    lpress <- tapply(lpres,lblo,f.scl)
    rpress <- rbind(rpress,cbind(lrep,lbo,lpress))
  }
  licf <- 
    model$assign!=match(blocks,attr(model$terms,"term.labels"),nomatch=-1)
  lcf <- rcomp[,licf]
  lcfmn <- apply(lcf,2,f.loc)
  lcfse <- apply(lcf,2,f.scl)
  if (!lboot) lcfse <- sqrt(lnbl-1)*lcfse
  lcfsum <- cbind(mean=lcfmn,se=lcfse)
  lpredlsig <- tapply(log(rpress[,3]),rpress[,2],f.loc)
  lpsbl <- rep(NA,lnbl)
  names(lpsbl) <- levels(lblocks)
  lpsbl[as.numeric(names(lpredlsig))] <- lpredlsig
  lpsmn <- f.loc(lpsbl)
  rr <- list(coef=lcfsum, psig=exp(lpsmn), coefblocks=lcf,
             psigblocks=exp(lpsbl), fit=rfit)
                          ...
                          weights, subset, na.action, contrasts, env)
{
  ...
  if(any.nonpen)
    index[matches] <- NA
  attr(index,'grpnames') <- attr(Terms,'term.labels')
  list(x = x,
       y = y,
       w = w,
       off = off,
...
## ===========================================================================
## ===========================================================================
legendr <- function(x=0.05,y=0.95,...) {
  lusr <- par("usr")
