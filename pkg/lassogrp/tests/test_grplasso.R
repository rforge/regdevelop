...
fit <- lasso(y ~ Pos.1 * Pos.2, data = splice, lambda = 25, control = control,
...
fit.A <- lasso(y ~ Pos.1 * Pos.2 * Pos.3, nonpen = ~ 1, data = splice,
                  standardize = TRUE, lambda = 61:1,
                  contrasts = contr.A, control = control)
fit.B <- lasso(y ~ Pos.1 * Pos.2 * Pos.3, nonpen = ~ 1, data = splice,
                  standardize = TRUE, lambda = 61:1,
...
fit1 <- lasso(y ~ ., data = splice, lambda = 0.5 * lambda.max,
...
fit.maxiter <-
  lasso(y ~ Pos.1 * Pos.2, data = splice, lambda = c(1, 0.1),
