...
\title{\vspace*{-10mm}
The R-Function \T{regr} and Package \T{regr} for an Augmented 
Regression Analysis}
\author{Werner A. Stahel, ETH Zurich}
\maketitle

\begin{abstract}\noindent
  {\normalsize\textbf{Needs revision! This is mostly the description of
      a preliminary version that included the functions in the new package 
      'plgraphics'}}
...
mframe()
userOptions(project="regr documentation")
...
The reason why it is not on CRAN and why it is called \T{regr} rather than 
...
<<taplot, source=F, echo=F>>=
r.blast <-
  regr(log10(tremor)~location+log10(distance)+log10(charge), data=d.blast)
plot(r.blast, plotselect=c(resfit=3, default=0), xvars=F, mf=c(1,1) )
...
plot(t.rr, plotselect=c(resfit=3, default=0), xvars=F, mf=c(1,1),
     ask=FALSE)
...
In \T{regr}, there is a function with more flexibility, called
\T{plmatrix}.
\begin{itemize}
\item 
