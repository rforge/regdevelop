...
regroptions(termcolumns="conventional")
...
%%\citeasnoun{StaW20} 
Stahel (2020) and Section \ref{sec:relevance} below.
<<regroutrel>>=
regroptions(termcolumns="relevance")
r.blast
...
\end{description}
In fact, three more columns are contained in \T{rr\$termtable}, but they are
...
in the \T{.regroptions} list and controled by the function 
\T{regroptions} in the same way that the usual options are
...
In \T{regr} output, they are shown in the Terms table by default. 
If the table gets too wide, the confidence limits may be suppressed by
modifying the \T{termcolumns} option in \T{regroptions}.
...
variables. 
These coefficients are also stored in \T{rr\$termtable} as the column 
\T{stcoef} and can therefore be seen as follows.
<<termtable>>=
...
\[
  \T{signif0} = \sqrt{\T{F value}\;/\;q_{df1,df2}}
\]
...
\item[termcolumns]     names of columns of the \T{termtable} component 
  ...
  alternatively, a single string, either \T{"relevance"} or \T{"conventional"}
