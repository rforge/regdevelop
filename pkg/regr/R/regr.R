...
##
  dr1 <- if (add) { ## ------------ add
    ...
    li <- row.names(ldata)%in%rownames(cbind(object$residuals))
    ...
    lidropNA <- li & !apply(is.na(ldata[,lvars]),1,any)
    lnobs <- sum(lidropNA)
    lnrd <- sum(li)
    ...
    if (lnobs!= lnrd) {
      ...
      if(!is.null(lsubs <-
        ...
        lnsubs <- rep(FALSE,length(lidropNA))
        lnsubs[lsubs] <- TRUE
        lidropNA <- lidropNA &!lnsubs
      }
      lfc$subset <- lidropNA
...
##-            lfc[[1]]
##-       object <- update(object, subset=lidropNA)
#      environment(object$call$formula) <- environment()
##-       class(object) <- setdiff(class(object), "regr")
    }
    if (!all(lidropNA)) { ## needed if NA's have been generated by transformations
      lfc$subset <- lidropNA
      object <- eval(lfc, envir=environment(formula(object)))
    }
...
getUseroption <- function (x, default = NULL)
{
  if ((!is.atomic(x))||!is.character(x)) {
    warning(":getUseroption: argument 'x' must be of mode character")
    return(NULL)
  }
  if (length(x)>1) {
    warning(":getUseroption: argument 'x' has length >1. Only 'x[1]' is used.")
    x <- x[1]
  }
  if (is.null(default))
...
## -----------------------------------------------------------------------
useroptions <-
  function (x=NULL, default=NULL, list=NULL, useroptions = NULL,
            assign=TRUE, ...)
{ ## 
  lnewo <- loldo <-
    if (is.null(useroptions)) {
    if (exists(".useroptions", where=1)) get(".useroptions", pos=1)
    else  useroptionsDefault
    } else useroptions
  largs <- c(list, list(...))
  lop <- c(names(largs))
  ##
  if (!is.null(x)) {  ## asking for options
    if(!is.character(x)) {
      warning(":useroptions: First argument 'x' must be of mode character")
      return(NULL)
    }
  ...
  if (length(default) && u.notfalse(default)) { ## get default values
    ...
    if (!is.character(default))
      stop("!useroptions! Argument 'default' must be of mode character or logical")
    if (default[1]=="all") return(useroptions(list=useroptionsDefault, assign=assign))
    ## resets all available components
    if (default[1]=="unset")
      return(useroptions(list=useroptionsDefault[names(useroptionsDefault)%nin%
                                             names(loldo)],
                       assign=assign) )
    if (any(default!="")) {
      llopt <- useroptionsDefault[default[default%in%names(useroptionsDefault)]]
      return( useroptions(list=llopt) )
    }
  }
  ## set options
  ## check
  largs <- check.arg(list=largs, fname="useroptions")
  if (length(largs))  lnewo[names(largs)] <- largs
  if (assign) assign(".useroptions", lnewo, pos=1)
...
.useroptions <- useroptionsDefault <- list(
  stamp=1, show.termeffects=TRUE, digits = 3,
...
## -----------------------------------------------------------------------
check.arg <- function(optname, value, list=NULL, fname="") {
