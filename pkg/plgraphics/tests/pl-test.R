...
par("mar")
attr(.ploptions, "oldpar")
ploptions(default="mar", setpar=TRUE)
stopifnot(all(par("mar")==ploptionsDefault$mar))

par(attr(.ploptions, "oldpar"))

## plpar
plmframes(2,1)
ploptions(default="all", setpar=TRUE)
par(lwd=1)
t.plo <- plpar(mar=c(3,3,6,2), lwd=2)
stopifnot(
  all.equal(attr(t.plo,"oldpar"),
            c(list(lwd=1), ploptionsDefault[c("mar","oma","mgp","cex")],
              list(mfg=par("mfg")))
            ))
par(mar=c(2,2,5,2))
plyx(Sepal.Width~Sepal.Length, data=iris) ## margins according to ploptions
par("mar") ## paramteres have been recovered
mtext("wrong place for mtext",3,1, col="red")  ## margins not appropriate for active plot
t.usr <- par("usr")
mtext("still wrong",3,0, col="red")  ## margins not appropriate for active plot
points(8,4.5, pch="X", col="red", xpd=TRUE) ## with xpd=F, the point is not shown
plaxis(4)
par(usr=t.usr)
...
t.plo <- plpar() ## get margin parameters from .plargs 
  ## generated by the last pl graphics call
par("mar")
mtext("here is the right place",3,1, col="blue")
points(t.usr[2]-1,t.usr[4], pch="O", col="purple", cex=2)
...
plpoints(col="blue", cex=2) ## gets the coordinates from .plargs
