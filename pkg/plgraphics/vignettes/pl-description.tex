...
\item
  a group \T{refline}, where \T{refline} can be logical or \T{==2},
...
The high level pl functions set graphical parameters (as specified by calling
\T{par}) according to their needs and the pl options (specified by calling
\T{ploptions}). When they end, they reset the parameters to the previous values
unless the argument \T{keeppar} is \T{TRUE}.
Since most often, the parameters determining the width of the plot margins are
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in plpar(): could not find function "{}plpar"{}}}\begin{alltt}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in par(attr(t.plo, "{}oldpar"{})): object 't.plo' not found}}\begin{alltt}
\hlkwd{par}\hlstd{(}\hlstr{"mar"}\hlstd{)}
\end{alltt}
\begin{verbatim}
## [1] 2 2 5 2
