...
\hlkwd{data}\hlstd{(d.cheese)}
\hlstd{t.r} \hlkwb{<-} \hlkwd{survreg}\hlstd{(}\hlkwd{Tobit}\hlstd{(Anzahl,} \hlnum{1}\hlstd{,} \hlkwc{log}\hlstd{=}\hlnum{TRUE}\hlstd{)}\hlopt{~}\hlstd{Temp}\hlopt{+}\hlstd{Bakt}\hlopt{+}\hlstd{Konz,} \hlkwc{data}\hlstd{=d.cheese,} \hlkwc{x}\hlstd{=T,}
               \hlkwc{dist}\hlstd{=}\hlstr{"gaussian"}\hlstd{)}
\hlkwd{plot.regr}\hlstd{(t.r,} \hlkwc{plotselect}\hlstd{=}\hlkwd{c}\hlstd{(}\hlkwc{default}\hlstd{=}\hlnum{0}\hlstd{,} \hlkwc{resfit}\hlstd{=}\hlnum{1}\hlstd{),} \hlkwc{xvar}\hlstd{=}\hlnum{FALSE}\hlstd{,} \hlkwc{mf}\hlstd{=}\hlkwd{c}\hlstd{(}\hlnum{1}\hlstd{,}\hlnum{1}\hlstd{),}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
  \hlkwd{data}\hlstd{(d.babysurv)}
  \hlstd{rr} \hlkwb{<-} \hlkwd{glm}\hlstd{(Survival}\hlopt{~}\hlstd{Weight}\hlopt{+}\hlstd{Age}\hlopt{+}\hlstd{Apgar1,}\hlkwc{data}\hlstd{=d.babysurv,}\hlkwc{family}\hlstd{=binomial)}
  \hlkwd{plmfg}\hlstd{(}\hlnum{1}\hlstd{,}\hlnum{2}\hlstd{)}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in simresiduals.glm(x, lnsims, glm.restype = glm.restype): :simresiduals: binomial distribution with unsuitable response.\\\#\#\ \  No residuals simulated}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in order(as.numeric(lgrp), lx): argument lengths differ}}\end{kframe}
...
\item
  ...
  \T{basic.pch, basic.cex, basic.cex.plab, basic.lty, basic.lwd, basic.col}.\\
...
\item
  a group \T{refline}. A \T{refline} is set iby some high level 
