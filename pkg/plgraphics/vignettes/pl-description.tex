...
\begin{alltt}
  ...
  \hlkwd{plyx}\hlstd{(O2}\hlopt{+}\hlstd{T}\hlopt{+}\hlstd{ra}\hlopt{~}\hlstd{date,} \hlkwc{data}\hlstd{=d.river[t.i} \hlopt{&} \hlstd{d.river}\hlopt{$}\hlstd{hour}\hlopt{==}\hlnum{14}\hlstd{,],}
    \hlkwc{smooth.par}\hlstd{=}\hlnum{0.5}\hlstd{,} \hlkwc{smooth.xtrim}\hlstd{=}\hlnum{0.03}\hlstd{)}
\end{alltt}
\end{kframe}
