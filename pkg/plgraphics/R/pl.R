  ...
  if (length(i.def(x, NULL, TRUE, NULL))) {
    if (is.atomic(x)&&is.character(x))  ## names of variables
      ...
      if (is.data.frame(x)) {
        ...
        data <- if (is.null(data)) x else cbind(x, data) ## !!! check nrow
      } else {  ## formula
...
genvarattributes <-
  function(data, vnames = NULL, vcol = NULL, vlty = NULL, vpch = NULL,
           ...
           plscale = NULL, replace = FALSE, ploptions = NULL)
{
    ...
    if(!is.character(varlabels))
      ...
      if (length(names(varlabels))==0) {
        if (length(varlabels)==ncol(data))
          llb <- setNames(varlabels, colnames(data))
...
##      class(lvvv) <- setdiff(class(lvv), c("matrix","data.frame"))
      ...
      if (is.numeric(lvvv)) {
        ...
        if (lir && u.notfalse(lirgv <- lirg[[lv]]) ) {
          ...
          if (!lIplrg) {
            lnouter <- c(attr(lvv, "nouter"), 0, 0)[1:2]
        ...
        else ## plrange from attributes
          if(sum(lnouter==0)) lrg <- attr(lvv, "plrange")
  ...
  function(x, plscale = "log10", ticksat = NULL, n = NULL, logscale = NULL)
{
  ...
  as.character(substitute(plscale)) ## !!!??? when called from a function, ...
  if (lfn=="") return(x)
  ...
  if (is.null(ticksat)) {
    ticksat <- prettyscale(lx, lfn, n=ln, logscale=logscale)
  } else {
...
## =====================================================================
prettyscale <-
  function(x, plscale = "log10", inverse = NULL, n = NULL, logscale = NULL)
{
  ## generate ticks for transformed scale
  lf.pretty <- function(i, x) {
  ...
  as.character(substitute(plscale)) ## !!!??? when called from a function, ...
  if (is.character(plscale)) plscale <- get(plscale)
  lx <- plscale(x) ## the transforming function given by the argument
  ln <- i.def(n, i.getploption("tickintervals"))
  ln1 <- ln[1]
  ln2 <- if (length(ln)>=2) ln[2] else ln1+2 ## ceiling(n/2)
  ...
  if (lfn=="sqrt")  inverse <- function(x) x^2
  if (lfn=="qnorm") inverse <- pnorm
  if (is.null(inverse)) {
    inverse <- attr(plscale, "inverse")
    if (is.null(inverse))
      stop("!prettyscale! Back transformation function missing")
  }
  if (is.character(plscale)) plscale <- get(plscale)
  if (is.character(inverse)) plscale <- get(inverse)
  ## ---
  lrg <- range(lx)
  ld <- diff(lrg)/(ln1-1)
  lk <- c(-2,0:(ln1-1),ln1+1)
  lk <- lk+0.01*(lk-mean(lk))/ln1 ## correction needed for (most probably) breaking ties
  ltr <- lrg[1]+ld*lk
  lat <- rep(NA, ln1)
  lrw <- inverse(ltr)
  for (li in 1:ln1) {
    lrwx <- lrw[li+c(0,2)]
    lpr <- pretty(lrwx+diff(lrwx)*c(-1,1), n=ln2)
    lat[li] <- la <- lpr[which.min(abs(lpr-lrw[li+1]))]
  }
  lat <- unique(lat)
           ...
           doc=NULL, show=TRUE, plargs = NULL, ploptions = NULL, ...)
{
  ...
  if (length(ploptions)==0) ploptions <- plargs$ploptions
  lparcex <- par("cex")
  lop <- c(par(cex=i.getploption("cex")*lparcex), par(mar=par("mar")))
  on.exit(par(lop))
...
## ---------------------------------------------------------
##- i.lineout <- function(z, zz, lim, upper, after) {
##-   ## determines indices before which a point on the limit is needed
##-   ## and the "other" coordinate
##-   lout <- if(upper) z>lim  else  z<lim
##-   li0 <- if (after) which(c(FALSE,lout) &! c(lout, TRUE))
##-          else which(c(lout,FALSE) & !c(TRUE,lout))
##-   lz0 <- z[li0-1]
##-   lr <- (lim-lz0)/(lzd <- z[li0]-lz0)
##-   lzz <- zz[li0-1]+lr*(zz[li0]-zz[li0-1])
##-   lii <- order(c(1:length(z), li0-0.5))
##-   cbind(z=c(z,rep(lim,length(li0)))[lii], zz=c(zz,lzz)[lii])
##- }
...
## -----------------------------------------------------------------
plbars <-
  function(x, y, plargs = NULL, ploptions = NULL)
{
  if (length(plargs)==0) plargs <- get(".plargs", globalenv())
  if (length(ploptions)==0) ploptions <- plargs$ploptions
...
## -----------------------------------------------------------------
plsmooth <-
  function(x, y, band=NULL, power = NULL, plargs = NULL, ploptions=NULL)
{
  if (length(plargs)==0) plargs <- get(".plargs", globalenv())
  if (length(ploptions)==0) ploptions <- plargs$ploptions
...
## --------------------------------------------------------------
plsmoothline <-
  function(smoothline, x, y, plargs = NULL, ploptions = NULL, ...)
{
  if (length(plargs)==0) plargs <- get(".plargs", globalenv())
  if (length(ploptions)==0) ploptions <- plargs$ploptions
...
{ ## get data for plotting, collect and check arguments
  ...
  lres <- residuals(x)
  lcq <- i.getplopt(condquant) ## should be , ploptions but this is not yet avail.
...
##-     lmmar[3] <- i.def(i.getploption("mar")[3], 0.5) ##!!!
  ...
  lresplab <-
    if (lmxres>0 & stdresid)
      apply(lstdres, 2, function(x) plmark(lmxres, x, plabel=plargs$plabel) )
    else NULL
## -------------------------------------------
...
##-     lcall <- lcall[setdiff(names(lcall), i.argPlregr)]
    ...
    ##            list(fit=TRUE, hat=TRUE, stdresiduals=TRUE)  )
    mode(lcall) <- "call"
  ...
  if (any(c("absresfit","qq","leverage")%in%names(lplsel))) {
    ...
    for (lj in seq_len(lmres)) {
      ...
      if (plargs$smresid) { ## !!! transfer to plregr.control
        ...
        lstdres[,lj] <- lfsmr * lstrratio[,lj] ## !!! * f(leverage of smooth)
        if (lIcq) {
...
##               if (lnplsel<=2) lnplsel else c(2)
  ...
  ## reduce  mar[3]
  ploptions$mar <- pmax(i.getploption("mar")-c(0,0,1.5,0), 0.5)
    ...
    for (liplot in 1:length(lplsel)) {
      ...
      if(lpls=="yfit") {
        ...
        if (lpllevel>0) { ## !!! condquant bereinigen 
          ...
          ly <- lpldata[,lyvar]
          for (lj in seq_len(lmres)) {
            plframe(ly[,lj],lfit[,lj], mar=lmar, ploptions=ploptions)
            if (lpllevel>1)
      ...
      if(lpls=="resfit") {
        for (lj in seq_len(lmres)) {
          plframe(lfit[,lj], lres[,lj], mar=lmar, ploptions=ploptions)
      ...
      if(lpls=="absresfit")
        if(length(lstdres)) {
          ...
          for (lj in seq_len(lmres)) {
            ...
            ##
            plframe(lfitj, labssrj, ylab=labsresname[lj],
                    mar=lmar, ploptions=ploptsmod)
            if (lpllevel>1) {
              lsimabsres <- if (lnsims) abs(lsimstdres) else NULL
      ...
      if(lpls=="absresweights") {
        if (length(lweights)!=lnr)
          ...
          for (lj in seq_len(lmres)) {
            ...
            plframe(lwg, labssrj, ylab=lrlab[lj], mar=lmar, plargs=plargsmod)
      ...
      if ((!is.na(lpllevel))&&lpllevel>0) {
        ...
        if (diff(range(llev,na.rm=TRUE))<0.001)
          ...
          llevpl <- genvarattributes(
            data.frame(leverages=plcoord(llev, c(0,0.5), ploptions=ploptions)) )[,1]
...
## =======================================================================
plmatrix <-
  function(x, y=NULL, data=NULL, panel=plpanel, panelargs = plargs, 
...
##-   if (length(lcall)!=length(lcl)) stop("bug")
  ...
  lpanel <- panel
  if (is.character(lpanel)) get(lpanel)
  if (!is.function(lpanel)) {
    warning(":plmatrix: 'panel' not found. Using 'plgraphics::plpanel'")
    lpanel <- plgraphics::plpanel
  }
...
##-       plargs$pldata[,"(smooth.group)"] <- pldata[,"(group)"]
  ...
  lipanelargs <-
    intersect(names(as.list(args(lpanel))), c("indx","indy","panelargs"))
##
##-   ## log
...
##          lpanel(v1,v2, indx=jd1, indy=jd2, plargs=plargs)
        }
        else {
          lv0 <- as.numeric(v1)
...
##-   lcnm <- i.def(names(lcall), names(lcl))
  ...
  ## ---
  if (frame) plframe(x,y, ploptions=ploptions, setpar=FALSE)
    ...
    ...)
{
  ...
  ly <- pldata[,i.def(attr(pldata,"yvar")[1],2)]
...
## size
  ...
  lpanel <-
    function(xx, yy, indx, indy, pch, col, plab, lwd, zz, ...) {
      ...
      ly <- attr(yy, "plcoord")
      if (is.null(ly)) ly <- yy
      plpoints(xx, yy, plargs=plargs, setpar=FALSE)
