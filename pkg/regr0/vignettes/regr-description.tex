...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# The following object is masked \_by\_ '.GlobalEnv':\\\#\# \\\#\#\ \ \ \  UserOptions}}
...
\hlstd{d.blast}\hlopt{$}\hlstd{location} \hlkwb{<-} \hlkwd{factor}\hlstd{(d.blast}\hlopt{$}\hlstd{location)}
\hlstd{r.blast} \hlkwb{<-} \hlkwd{regr}\hlstd{(}\hlkwd{logst}\hlstd{(tremor)} \hlopt{~} \hlstd{location} \hlopt{+} \hlkwd{log10}\hlstd{(distance)} \hlopt{+} \hlkwd{log10}\hlstd{(charge),}
...
## Multiple R^2:  0.798    Adjusted R^2:  0.793    AIC:  -1409.04 
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlkwd{names}\hlstd{(r.blast}\hlopt{$}\hlstd{termtable)}
\end{alltt}
\begin{verbatim}
...
\end{verbatim}
\begin{alltt}
\hlkwd{userOptions}\hlstd{(}\hlkwc{termcolumns}\hlstd{=}\hlkwd{c}\hlstd{(}\hlstr{"coef"}\hlstd{,} \hlstr{"stcoef"}\hlstd{,} \hlstr{"df"}\hlstd{,} \hlstr{"signif"}\hlstd{,} \hlstr{"p.symb"}\hlstd{))}
...
\T{add1} drops the rows with any missings in the variable used by the
\T{scope} before calculations.\\
\T{step} uses these two functions for the two directions.
...
be elevated. The arguments \T{plotselect} and \T{xvars} allow for 
...
\includegraphics[width=\maxwidth]{figure/taplot-1} 

\includegraphics[width=\maxwidth]{figure/taplot-2} 

\includegraphics[width=\maxwidth]{figure/taplot-3} 

\includegraphics[width=\maxwidth]{figure/taplot-4} 
...
\end{alltt}
\end{kframe}
\includegraphics[width=\maxwidth]{figure/taordered-1} 

\includegraphics[width=\maxwidth]{figure/taordered-2} 

\includegraphics[width=\maxwidth]{figure/taordered-3} 

\includegraphics[width=\maxwidth]{figure/taordered-4} 
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in plot.xy(xy.coords(x, y), type = type, ...): "{}pcol"{} is not a graphical parameter}}
...
{\ttfamily\noindent\color{warningcolor}{\#\# Warning in plot.xy(xy.coords(x, y), type = type, ...): "{}pcol"{} is not a graphical parameter}}\end{kframe}
