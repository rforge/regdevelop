...
Note that for factors with only two levels, a single coefficient
determines its effect. 
If ``treatment'' contrasts are used, the coefficient equals the effect
of a change from the first to the second level of the factor.
In fact, such a factor could be replaced by the corresponding
0-1 variable.
\T{regr} does this for \ul{logical explanatory variables.}
(Factors with two levels are not converted because this would
mean that contrasts, other than treatment contrasts, would be 
defined inconsistently once only two levels remain after subsetting a
dataset.)

The test performed for factors, which is shown 
in the \T{termtable} table by the \T{p.value} and \T{signif} entries, 
