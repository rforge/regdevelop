...
\begin{document}
\input{regr-description-concordance}
...
\end{itemize}
The \T{regr} function uses some default arguments that differ from those
used by the functions just mentioned, see Appendix \ref{app.regr}.
...
\textbf{Note.} 
The fact that the function \T{logst} adapts to the dataset causes
problems when predicting with ``new data'', unless the \T{threshold}
argument is set explicitly.

The \T{regr} method of \T{predict} solves this problem by getting the
threshold from the \T{model} component of the \T{regr} object,
but with standard \T{predict} methods, the threshold will be determined on
the basis of the \T{newdata}, which distorts the regression formula.
...
\appendix
\section{Appendix}
\subsection{Details about \T{regr}}
\label{app.regr}
Here are some features of the function \T{regr} which differ from 
the usual behavior of R functions.
\Itm
Contrasts: The default contrasts for factors are ``weighted sum''
contrasts, that is, \T{contr.wsum} is applied.
\Itm
Factors: Unused levels are dropped. 
By default, \T{NA}s are converted into a level \T{"NA"}.


Note that logical variables are treated as factors with levels 
\T{FALSE} and \T{TRUE} as in \T{lm}.
This entails that contrasts are applied to them.
