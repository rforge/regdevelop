...
## terms with 1 coef
  lcont1 <- lcont <- which(table(lasg)==1)-1 
## vif --> R2.x
  lr2 <- NA
  if (vif) {
...
##
  ...
  if (length(dummycoef)==0)
    dummycoef <- c(getOption("show.dummy.coef"),TRUE)[1]
...
function(mfrow=NULL, mfcol=NULL, mft=NULL, row=TRUE, oma=c(0,0,2,1),
                 mar=getOption("mar"), mgp=getOption("mgp"), ...)
{
## Purpose:    par(mfrow...)
