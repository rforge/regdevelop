plot.regr <-
function(x, data=NULL, nmark=NULL, lab=NULL, cex.lab=0.7, 
         mf = NULL, mfcol=FALSE, mar=c(3,3,2,1), mgp=c(2,0.7,0),
         oma = 2*(prod(mf)>1), cex=par("cex"), ask = NULL,
         multnrows = 0, multncols=0, 
         lty = c(1,2,5,3,4,6), lwd = c(1,1,2,1,1.5,1), colors = c.env$colors.ra,
         pch=NULL, 
...
##  data
##  nmark      how many extreme positive and negative residuals should be
##             labeled?
...
## prepare objects needed for plotting
  ...
  x$na.action <- NULL
  lres <- cbind(residuals(x, if (lfgauss) "response" else glm.restype))
  ...
  if (is.logical(res.lim)&&!is.na(res.lim)) res.lim <- 
    if (res.lim)  apply(lres,2,robrange) else
      NULL
  if (!is.null(res.lim))
...
##-   ldfres <- x$df.residual
  ...
  lsigma <- x$sigma
  if (length(lsigma)==0) lsigma <- sqrt(apply(lres^2,2,sum)/ldfres)
#  lsigma <- x$sigma
## weights
  if (lwgt) {
    lwgts <- lweights[lnna]
...
## standardized residuals
  ...
  lstres <- if (length(lstres)>0)
    cbind(x$stres)[lnna,,drop=FALSE] else {
      if (all(lsigma>0)) 
        lres[lnna,]/outer(ifelse(lhat>=1,1,sqrt(pmax(0,1-lhat))),lsigma) else
        lres[lnna,]
    }
...
## labels
  ...
  if (!is.character(lab)) lab <- as.numeric(lab) # factors: as.numeric
  if (length(lab)>1&&length(lab)!=ln) {
    warning(":plot.regr: argument  lab  has unsuitable length")
    lab <- NULL
  }
  llabels <- if (length(lab)==0) lrown else rep(lab, length = ln)
  if (length(nmark)==0||is.na(nmark))
    nmark <- if (length(lab)>1) lna else ceiling(sqrt(lna)/2)
  if (nmark==0) {
    llab <- rep(lpch, length=sum(lnna))
    llabna <- NA
  } else  {
    llab <- llabels[lnna]
    if (nmark<ln/2) {
      li <- if (lmult)  order(lresmd[lnna])[1:(lna-2*nmark)]  else 
      order(lstres[lnna])[(nmark+1):(lna-nmark)]
...
## prepare graphical elements
  ...
  colors <- if (length(colors)==0) 
    c("black","gray","blue","cyan","red","magenta")  else  rep(colors,length=6)
...
##  y on fit
  if(lpls=="yfit") {
    ...
    if (lpllevel>0) {
      ...
      lsimr <- c(lf)+lsimres
      i.plotlws(lf,ly, "fitted",lyname,main,outer.margin,cex.title,
                colors,lty,lwd,lpch, 
    ...
    if (lpllevel>0) 
      i.plotlws(lf,lresa, lfname,lrname,main,outer.margin,cex.title,
              ...
              lpllevel>1,smooth,lsmpar,smooth.iter, ylim=res.lim,
                reflinex=mean(lf),refliney=-1,lnsims=lnsims, simres=lsimres)
  }
## --- 
  if(lpls=="tascale")  {
...
##-       qqnorm(lstres[,lj], ylab = lstrname[lj], main="", col=colors[1])
      ...
      if (lpllevel>2&lnsims>0&&length(lsimstres)>0) {
        ...
        for (lr in 1:lnsims) {
          lines(lxx,sort(lsimstres[,lr]),lty=lty[4], lwd=lwd[4], col=colors[4])
        }
...
## --- leverage plot. If weight are present, use "almost unweighted h"
  ...
  if ((!is.na(lpllevel))&&lpllevel>0) {
    if (diff(range(lhat,na.rm=TRUE))<0.001)
      ...
      llabh <- llab
      if (nmark>0 & nmark<ln/2) {
      lor <- order(lhat)
      li <- lor[sum(lnna)-nmark+1:nmark]
      ...
      for (lj in 1:lmres) {
        i.plotlws(lhat, lstres[,lj], lhattit, lstrname[lj],
              ...
              FALSE,smooth,lsmpar,smooth.iter) 
...
## plot residuals vs. explanatory variables by calling plresx
  ...
  if (lxpl | is.na(lseq)|lseq) {
    plresx(x, partial.resid=TRUE, glm.restype = glm.restype, lab=llabs,
           ...
           wsymbols=lwsymbols, symbol.size=symbol.size, pch=pch, ...)
  }
## --- end
  par(loldpar)
  "plot.regr done"
