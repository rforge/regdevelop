                 ...
                 model=TRUE, vif=TRUE, termtable=TRUE, ...)
{
  ...
  lreg <- if (method=="rlm") {
    require(MASS)
    rlm(formula, data=data, weights=.Weights, model=model, ...)
  } else if (method=="nls") {
                 ...
                 model=TRUE, vif=TRUE, termtable=TRUE, ...)
{
  ...
  lreg <- # if (length(data[,".Weights"])>0)
    polr(lfo, data=data, weights=.Weights, Hess=TRUE, ...)
...
fitted.polr <- function(object, type="link", na.action=object, ...) {
  lfit <- if (pmatch(type,"link",nomatch=0))
    c(model.matrix(object)[,-1]%*%object$coefficients) else
    object$fitted
...
predict.polr <-
  ...
  ## type linear.predictor added by WSt, newdata=NULL
{
...
#  if (length(lsig)==0) lsig <- summary(object)$sigma
  ...
  lst <- t.y[,2]
  li <- lst!=1
  llim <- cbind(lres[li],c(Inf,0,-Inf)[lst[li]+1])
  lr <- condquant(llim, dist, lsig)
  lrr <- matrix(lres,length(lres),ncol(lr),
                dimnames=list(row.names(ly),colnames(lr)))
  lrr[,'prob'] <- 0
  lrr[li,] <- lr
...
## median point and prediction for it
  ...
  lprm <- c(lf.predict(object, newdata=xm))
  lny <- length(lprm)
##  expand to matrix
  if (xfromdata) {
    lx <- ldata
  ...
  for (lv in lvars) {
    if (xfromdata) {
      ...
      if (is.factor(ldv)) { # ---
        ...
        lx[1:lnl,lv] <- ldx
        ##
        lpr <- lf.predict(object, newdata=ld, se.fit = se)
        if (se) {
          lc <- lpr$fit
    ...
    lpr <- lf.predict(object, newdata=ld, se = se)
    if (se) {
      lcomp[,lv,] <- lpr$fit
      lcse[,lv,] <- lpr$se.fit
    } else lcomp[,lv,] <- lpr
...
#        lines(lxx, sort(lstres[,lj]), col=colors[1])
      ...
      if (is.character(llab))
        text(lxx,lyy,llab[li], cex=cex.lab, col=colors[1]) else
        points(lxx,lyy, pch=lpch, cex=cex.lab, col=colors[1])
...
i.plotlws <- function(x,y, xlab="",ylab="",main="", outer.margin=FALSE,
  cex.title=1.2, colors=1:6, lty=1:6, lwd=1, pch=1, pty="p", lab="+", cex.lab=1,
...
## data
  ...
  if (nrow(ldata)!=nrow(lres)) {
    ...
    if (length(li)==0||any(is.na(li))) 
      ...
      ldata <- ldata[li,]
      if (lwgt) {
        if (length(lwgts)==nrow(ldata)) lwgts <- lwgts[li]
        else {
          if (length(lwgts)!=nrow(lres)) {
            lwgt <- FALSE
            warning(':plresx: unsuitable weights')
    }}}}
  }
## weight symbols
  if (lwgt) {
...
##  ldfres <- x$df.residual
  ...
  is.fac <- sapply(ldata[,vars,drop=FALSE],
                   function(x) { is.factor(x)|(length(unique(x))<=2) } )
...
##-   x$call$data <- as.name("ldt")
  if (length(tin)>0) {
    lcmp <- fitcomp(x, ldt, xfromdata=FALSE, se=se)
    lcompx <- lcmp$x
    ...
    if (addcomp) {
      if (length(ldt)==0&&(sum(lnna)!=nrow(ldt))) # length(res)
        stop("!plresx! BUG: number of residuals != nrow(comps)")
      lcompdt <- fitcomp(x, ldt, vars=tin, xfromdata=TRUE)$comp
    }
  }
## ------------------------------------------------------------------
  if (inherits(x,"mlm")) {
...
##-             lines(lowess(v2,v1), col=clrsmooth) }
    lpanel <- function(xx, yy, jx, jy, pch, clr) {
      ...
      i.plotlws(xx,yy, "","","", TRUE, cex.title, colors, lty, lwd, lpch,
...
## factors
      ...
      i.plotlws(xx, rr, xlab = xlabs[lv], ylab = ylabs[lv],
        ...
        colors, lty, lwd, lpch, lpty, llab, cex.lab, ltxt,
        lwsymbols, lwgt, lwgts, liwgt, lsyinches, 
...
## range
  ...
  if(is.matrix(range.)) rg[,match(colnames(range.),vnames)] <- range. else 
...
## print some rows (and columns) of a matrix or data.frame
  ...
  if (l.nr<=nrow.+first)  l.dc <- format(ldata[,l.ic])  else {
    l.ir <- c(1:first,seq(first,by=l.nr%/%nrow.,length=nrow.)[-1],l.nr)
...
logst <- function(data, mult=1)
{
  ...
  ldp <- data[data>0]
  if(length(ldp)==0) {
    warning(":logst: no positive data. I return the original values")
    return(data)
  }
  lq <- quantile(ldp,probs=c(0.25,0.75),na.rm=TRUE)
  lc <- lq[1]^(1+mult)/lq[2]^mult
  li <- which(data<lc)
  if (length(li)) 
    data[li] <- 10^(log10(lc) + (data[li]-lc)/(lc*log(10)))
  ldt <- log10(data)
  attr(ldt,"threshold") <- c(lc)
  ldt
}
## ===========================================================================
...
u.gmeth <- function(fn,mt)  getS3method(as.character(substitute(fn)),
