...
named by \T{rloptions("printstyle")} or \T{rloptions("termcolumns")}.
...
The function \T{rloptions} plays a role like \T{options} of standard R.
...
\item \T{printstyle}: selects the columns of a \T{termtable} 
  ...
  \item \T{relevance}: selects the columns determined by
    \T{rloptions("termcolumns.r")}, the important columns for inference
    based on relevance,
  \item \T{old}: columns in \T{rloptions("termcolumns.o")}, suitable for
...
{\small
\Tit{Detail.}\T{rloptions} accesses the list \T{rl.options} in the environment
  \T{rl.envir} that is generated when the package is loaded.
  The package's default setttings are available as \T{rl.optionsDefault}.  
}

<<rloptions>>=
t.opt <- rloptions(termcolumns.r=c("coef", "dropRls", "dropRls.symbol"))
rt
## restore the old options
rloptions(list = t.opt)
@ 
