...
common practice of Null Hypothesis Statistical Testing that is in turn
...
coefficient with the respective inference descriptors, confidence interval,
...
<<termtable>>=
  ...
  if(interactive()) {
    ...
    print(data.frame(rt)) ## avoid selection and preparation of columns 
    ## by 'print.inference'. This produces an extensive output.
  }
@ 
...
( r.termeff <- print(termeffects(r.blast), print=FALSE) )
...
\end{itemize}
The packages defaults can always be restored by typing
...
\T{print.printInference}, which in turn produces the output.
