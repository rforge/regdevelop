...
%\VignetteIndexEntry{'relevance: Calsulate Relevance and Significance Measures'}
...
\begin{abstract}\noindent
  ...
  They are based on the specification of a threshold of relevance and
  indicate whether a result is to be called (scientifically) relevant,
  negligible, or ambiguous.
  
  The package \T{relevance} calculates these measures for simple a
  comparison of groups as well as for many regression models 
  and allows for prinitng them in a decent manner.
...
\Tit{The problem.}
effect : parameter in a model

two groups

regression: coefficient

dropping a term, prediction

inference based on confidence interval for effect

\Tit{Short introduction to Relevance.}

need rel threshold

relevance: effect, divided by relevance threshold

inference: compare estimated relevance to rel threshold

by-product: significance
...
<<twogroups>>=
  ...
  twogroups(sleep[sleep$group == 1, "extra"], sleep[sleep$group == 2, "extra"])
@ 
...
\section{Options}
\Tit{Relevance threholds.}

\Tit{Selecting columns of \T{termtable} objects to be printed.}
